...
setup_bench = (suite, number) ->
  suite.add("#{number} publishes", (d) ->
    payload += 1
    publish_count = 0
    subscribed_count = 0
    topic = "bench/#{number}"
    ...
    pool.get (client) ->
      ...
      client.on 'suback', ->
        ...
        setup_request = (num) ->
          process.nextTick ->
            request.put url: "http://#{host}:#{http_port}/topics/#{topic}", json: { payload: num}

        setup_request(num) for num in [0..number]
...
# setup_bench(suite, 1)
# setup_bench(suite, 10)
# setup_bench(suite, 100)
# setup_bench(suite, 1000)
setup_bench(suite, 10000)
